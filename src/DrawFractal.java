...
/*************************************/
    ...
    if(lines.length == i){
        expandCapacity(); // Expands space within data structure
       }

       if(symbol.equals("[")){
           s.push(new CurrentPointInfo(x1, y1, drawingDir, length)); // Pushes symbols in order to draw
       }

       if(symbol.equals("]")){ // Pushes symbols in order to draw
           CP = s.pop();
       ...
       if(symbol.equals("-")){  // Draws angles when symbol appears in string
           drawingDir -= angle;
       }
       ...
       if(symbol.equals("F")){  // Symbols with rules which are replaced
           x2 = x1;
           x2 = x2 + Math.cos(drawingDir);
           y2 = y1;
           y2 = y2 + Math.sin(drawingDir);
           UpdateXminYminXmaxYmax(x1,y1,x2,y2); // X and Y values for lines
       ...
       if(symbol.equals("G")){ // Symbols with rules which are replaces
           x2 = x1;
           x2 = x2 + Math.cos(drawingDir);
           y2 = y1;
           y2 = y2 + Math.sin(drawingDir);
